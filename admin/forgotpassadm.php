...
<body>
    ...
    if(isset($_POST["reset"]))
    {
        ...
        $ambil = $connect->query("SELECT*FROM admin where admin_email='$email'");
        ...
        if($match==1)
        {
            $newpass = md5($email);
            $nphash = md5($newpass);
            $connect->query("UPDATE `admin` set `password`='$nphash' where admin_email='$email'");
            echo "<div class='alert alert-danger container'>
            <h3>Hi, <strong>".$name['nama_lengkap']."</strong></h3>
